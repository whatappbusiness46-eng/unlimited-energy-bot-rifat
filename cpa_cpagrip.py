...
# ============================================================
# DROP-IN POSTBACK / ADMIN HELPERS
# ============================================================

def build_cpagrip_postback_url(public_base_url: str) -> str:
    """URL to register as CPAGrip's global postback endpoint."""
    base = str(public_base_url or "").rstrip("/")
    return (
        f"{base}/postback/cpagrip"
        "?tracking_id={tracking_id}"
        "&payout={payout}"
        "&txid={transaction_id}"
        "&status={status}"
...
def handle_cpagrip_conversion(
    ...
    """
    Process a CPAGrip conversion and optionally credit the user's
    existing wallet.

    `credit_user` must be a project-specific callable:
        credit_user(user_id, reward, offer_id, txid)

    This avoids guessing the repository's balance field and prevents
    accidentally writing to the wrong wallet column.
    """
    ...
    try:
        ...
        logger.exception(
            "CPA conversion processed but wallet credit failed | user=%s",
            user_id,
        )
...
def cpa_offer_summary(offer: Dict[str, Any]) -> str:
    payout = _money(offer.get("payout", 0))
    incentive = "✅ Reward" if offer.get("incentive_allowed") else "ℹ️ No bot reward"
    return (
        f"🎯 {offer.get('title', offer.get('offer_id', 'Offer'))}\n"
        f"🌍 {offer.get('geo', 'WW')}\n"
        f"💵 Network payout: ${payout}\n"
        f"{incentive}"
    )
